...
Because Weacast web app is also released as a Docker image you can build it like this (`version_tag` being the current version number like 1.1.2)
```bash
docker build -t weacast/weacast:version_tag .
```
Then release it:
```bash
docker login
